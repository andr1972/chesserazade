...
NegamaxResult negamax(Board& board, int depth, int ply, int alpha, int beta,
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        // For each depth keep only the first N scored moves (table
        // below); drop the rest outright. PV-nodes need every move
        // because they aim at the exact best; non-PV-nodes only
        // prove a bound, so the speculative cut is safe. Quiet
        // bucket check (`score < 50'000`) spares the TT move,
        // captures (good and bad — both have ≥ 700'000), killers,
        // counter-move and promotions. Ties on-and-off with PVS
        // since without PVS there are essentially no non-PV-nodes
        // to prune on.
        //
        // Extended past the original 'depth ≤ 3' clamp now that
        // SEE-lite + counter-move + gravity make the first ~24-32
        // entries reliably contain the search-worthy quiets.
        // Earlier attempt with the same thresholds at depth ≤ 7
        // regressed — the difference is the ordering improvements
        // that landed in 64577e2 / 4c18460.
