...
std::size_t score_and_sort(const Board& board, const MoveList& legal,
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        // to warm up. Pure positional change (×8 to scale against
        // the [0, 16384] history range). `psqt_delta()` skips the
        // material/dispatch overhead the search-only path doesn't
        // need (material cancels for same-piece moves anyway).
        std::int32_t psqt_bonus = 0;
        if (!is_capture(m) && m.kind != MoveKind::Promotion) {
            const int delta =
                psqt_delta(m.moved_piece, m.from, m.to);
...
NegamaxResult negamax(Board& board, int depth, int ply, int alpha, int beta,
    ...
    if (need_static_eval) {
        static_eval = stop.use_incremental_eval
            ? board.evaluate_incremental()
            : evaluate(board);
        // King-safety term is computed per-call (depends on occupancy
        // after each move so can't be incrementally maintained).
        // BoardBitboard-only — mailbox path skips.
        if (const auto* bb = dynamic_cast<const BoardBitboard*>(&board)) {
            static_eval += king_safety_stm(*bb);
        }
    }
